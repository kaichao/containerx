# 参考文献

## QEMU相关
- [QEMU简介](https://www.cnblogs.com/lygin/p/16937983.html)
- [QEMU 概述](https://martins3.github.io/qemu/introduction.html)
- [How to Setup QEMU Output to Console and Automate Using Shell Script](https://fadeevab.com/how-to-setup-qemu-output-to-console-and-automate-using-shell-script/)
- [Standalone virtiofs usage](https://virtio-fs.gitlab.io/howto-qemu.html)
- [Sharing files with Virtiofs](https://libvirt.org/kbase/virtiofs.html)
- [https://wiki.qemu.org/Documentation/9psetup](https://wiki.qemu.org/Documentation/9psetup)
- [virtiofsd](https://gitlab.com/virtio-fs/virtiofsd)
- [QEMU Cheat Sheet for Full-System Simulation](https://wangziqi2013.github.io/article/2022/03/09/qemu-cheat-sheet.html)

## docker-qemu
- [Balena base images](https://docs.balena.io/reference/base-images/base-images/)
- [Boot ARM64 virtual machines on QEMU](https://ubuntu.com/server/docs/boot-arm64-virtual-machines-on-qemu)
- [Docker and QEMU: A Powerful Combination for Accelerating Edge Computing Development and Optimizing Code Compilation](https://medium.com/@nullbyte.in/docker-and-qemu-a-powerful-combination-for-accelerating-edge-computing-development-and-optimizing-42da00259a02)

## FreeBSD相关
- [runj: a new OCI Runtime for FreeBSD Jails](https://samuel.karp.dev/blog/2021/03/runj-a-new-oci-runtime-for-freebsd-jails/)
- [riscv/QEMU](https://wiki.freebsd.org/riscv/QEMU)
- [FreeBSD Containers and Orchestration](https://wiki.freebsd.org/Containers)
- [Docker on FreeBSD](https://wiki.freebsd.org/Docker)

## 其他
- [Building Multi-Arch Images for Arm and x86 with Docker Desktop](https://www.docker.com/blog/multi-arch-images/)
- [Running and Building ARM Docker Containers on x86](https://www.stereolabs.com/docs/docker/building-arm-container-on-x86)
- [Using VxWorks 7 Containers With Dockerhub on a Raspberry Pi 4 Model B Board](https://harmonicss.co.uk/vxworks/using-vxworks-7-containers-with-dockerhub-on-a-raspberry-pi-4-model-b-board/)
- [Deep Dark Fantasy](https://martins3.github.io/)
- [Linux Lab v1.0 中文手册](https://tinylab.org/pdfs/linux-lab-v1.0-manual-zh.pdf)
- [Alpine Download](https://dl-cdn.alpinelinux.org/alpine/)
- [Preparing Machine Images for qemu/KVM](https://www.youtube.com/watch?v=6ccpDwT1qnw)
- [virtio-fs A shared file system for virtual machines](https://www.youtube.com/watch?v=EIVOzTsGMMI)
- [virtio-fs – present and future](https://www.youtube.com/watch?v=BhNz6quX6Dw)
- 